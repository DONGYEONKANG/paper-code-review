...
class Model(nn.Module):
    ...
    def __init__(self, backbone: BackboneBase, num_classes: int, pooling_mode: ROIWrapper.Mode,
                 anchor_ratios: List[Tuple[int, int]], anchor_scales: List[int], rpn_pre_nms_top_n: int,
                 rpn_post_nms_top_n: int):
        super().__init__()

        # backbone에서의 Conv layer
        # lateral_layers, dealiasing_layers도 같이 가져온다.
    ...
    def forward(self, forward_input: Union[ForwardInput.Train, ForwardInput.Eval]) -> Union[ForwardOutput.Train, ForwardOutput.Eval]:
        ...
        p4 = self.lateral_c4(c4) + F.interpolate(input=p5, size=(c4.shape[2], c4.shape[3]), mode='nearest') # NN방법
        ...
        for p, anchor_size in zip([p2, p3, p4, p5, p6], [32, 64, 128, 256, 512]):
            p_anchor_objectnesses, p_anchor_transformers = self.rpn.forward(features=p, image_width=image_width,
                                                                            image_height=image_height)
            # Anchor box 생성
            p_anchor_bboxes = self.rpn.generate_anchors(image_width, image_height, num_x_anchors=p.shape[3],
                                                        num_y_anchors=p.shape[2], anchor_size=anchor_size).cuda()


            # NMS
            p_proposal_bboxes = self.rpn.generate_proposals(p_anchor_bboxes, p_anchor_objectnesses,
                                                            p_anchor_transformers,
        ...
        if self.training:
            ...
            anchor_objectnesses = anchor_objectnesses[anchor_sample_fg_indices]
            ...
            proposal_sample_fg_indices, proposal_sample_selected_indices, gt_proposal_classes, gt_proposal_transformers = self.detection.sample(
                proposal_bboxes, forward_input.gt_classes, forward_input.gt_bboxes)
            proposal_bboxes = proposal_bboxes[proposal_sample_selected_indices]
            proposal_classes, proposal_transformers = self.detection.forward(p2, p3, p4, p5, proposal_bboxes,
                                                                             image_width, image_height)
            proposal_class_loss, proposal_transformer_loss = self.detection.loss(proposal_classes,
                                                                                 proposal_transformers,
                                                                                 gt_proposal_classes,
                                                                                 gt_proposal_transformers)

            forward_output = Model.ForwardOutput.Train(anchor_objectness_loss, anchor_transformer_loss,
                                                       proposal_class_loss, proposal_transformer_loss)
        else:
            proposal_classes, proposal_transformers = self.detection.forward(p2, p3, p4, p5, proposal_bboxes,
                                                                             image_width, image_height)
            detection_bboxes, detection_classes, detection_probs = self.detection.generate_detections(proposal_bboxes,
                                                                                                      proposal_classes,
                                                                                                      proposal_transformers,
                                                                                                      image_width,
                                                                                                      image_height)
    ...
    class Detection(nn.Module):
        ...
        def __init__(self, pooling_mode: ROIWrapper.Mode, num_classes: int):
            ...
            self._transformer_normalizer_mean = torch.tensor([0., 0., 0., 0.], dtype=torch.float).cuda()
        ...
        def forward(self, p2: Tensor, p3: Tensor, p4: Tensor, p5: Tensor, proposal_bboxes: Tensor, image_width: int, image_height: int) -> Tuple[Tensor, Tensor]:
            ...
            for uk in unique_k:
                ...
                pool = ROIWrapper.apply(p, uk_proposal_bboxes, mode=self._pooling_mode, image_width=image_width,
                                        image_height=image_height)
                pools.append(pool)
        ...
        def sample(self, proposal_bboxes: Tensor, gt_classes: Tensor, gt_bboxes: Tensor) -> Tuple[
            ...
            gt_proposal_transformers = BBox.calc_transformer(proposal_bboxes,
                                                             gt_bboxes[proposal_assignments[selected_indices]])
        ...
        def loss(self, proposal_classes: Tensor, proposal_transformers: Tensor, gt_proposal_classes: Tensor,
                 gt_proposal_transformers: Tensor) -> Tuple[Tensor, Tensor]:
            ...
            proposal_transformers = proposal_transformers[
                torch.arange(end=len(proposal_transformers), dtype=torch.long).cuda(), gt_proposal_classes]
            ...
            smooth_l1_loss = F.smooth_l1_loss(input=proposal_transformers[fg_indices],
                                              target=gt_proposal_transformers[fg_indices], reduction='sum')
        ...
        def generate_detections(self, proposal_bboxes: Tensor, proposal_classes: Tensor, proposal_transformers: Tensor,
                                image_width: int, image_height: int) -> Tuple[Tensor, Tensor, Tensor]:
            proposal_transformers = proposal_transformers.view(-1, self.num_classes, 4)
